...
static unsigned char read_byte(char **ptr)
{
  ...
  unsigned char val;
  c1 = (*(*ptr)++); c2 = (*(*ptr)++);
  val = xdigitconv(c1) * 16 + xdigitconv(c2);
  checksum += val;
  return val;
}
...
{
  unsigned int val1 = read_byte(ptr);
  unsigned int val2 = read_byte(ptr);
...
static int read_line(void)
{
  ...
  for (i = 1; i <= len; i++) set_memb(addr++, read_byte(&strptr));
  read_byte(&strptr);
  if (type != 1 && checksum != 0) {
    printf("Bad checksum\n");
    return 1;
  }
